...
void TMCConfig_Init(void)
{
    ...
    xTaskCreate(TMCMonitor_Handle, "TMCMonitor", 128, NULL, 5, &s_tmcMonitorTask);
...
Bool TMCConfig_WriteSubdivision(Uint8 slaveAddr, Uint32 subdivision)
{
    ...
    Uint32 chopConf = 0;
    if (FALSE == TMCConfig_ReadData(slaveAddr, TMC_REG_CHOPCONF, &chopConf))
    {
        TRACE_ERROR("\n ###### TMC Read ERROR! After Writr ######");
        chopConf = 0x10000053;
    }
    ...
    TRACE_INFO("\n*****TMC Write 0x%08X to addr 0x6C", chopConf);
...
//    return TRUE;
    ...
    if(slaveAddr == 3)
    {
        TMCConfig_CurrentSet(slaveAddr, 0, 20, 4);               //搅拌电机 电流设置
    }
    else
    {
        TMCConfig_CurrentSet(slaveAddr, 0, 20, 4);              //电流设置
    }
    ...
    while(1)
    {
        switch(s_tmcMonitorStatus)
        {
            case TMC_INIT:              
                ...
                Printf("\n*****TMC Start Setting!");
                Printf("\n");
                if(FALSE == TMCConfig_MotorDriverInit(PumpManager_GetPump(0)))
                {
                    Printf("\n");
                    Printf("\n*****TMC 0 Init ERROR!,Please Restart after power off !");
                    Printf("\n");
                }
                else
                {
                ...
                if(FALSE == TMCConfig_MotorDriverInit(PumpManager_GetPump(1)))
				{
					Printf("\n");
					Printf("\n*****TMC 1 Init ERROR!,Please Restart after power off !");
					Printf("\n");
				}
				else
				{
