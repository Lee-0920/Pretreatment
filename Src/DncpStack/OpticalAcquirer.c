...
/**
 * @brief 查询采集PD电位器默认增益参数。
 * @return value Uint16 ，增益参数。
 * @see DSCP_CMD_OAI_SET_STATIC_AD_CONTROL_PARAM
 */
void OpticalAcquirer_GetGainControlParam(DscpDevice* dscp, Byte* data, Uint16 len)
{
    Uint16 value = DSCP_ERROR;

	value = StaticADControl_Get_Gain_DefaultValue();

    DscpDevice_SendResp(dscp, &value, sizeof(value));
}

/**
 * @brief 设置采集PD电位器默认增益参数。
 * @details 增益参数器将根据设置的参数设置器件，并将参数永久保存在FLASH。
 * @param value Uint16 ，增益参数。
 * @return 状态回应，Uint16，支持的状态有：
 *  - @ref DSCP_OK  操作成功；
 *  - @ref DSCP_ERROR 操作失败；
 */
void OpticalAcquirer_SetGainControlParam(DscpDevice* dscp, Byte* data, Uint16 len)
{
    Uint16 ret = DSCP_OK;
    Uint16 size = 0;
    Uint16 value = 0;
    ...
    if (sizeof(value) == len)
    {
		...
		StaticADControl_Set_Gain_DefaultValue(value);
		ret = DSCP_ERROR;
    }
    else
    {
