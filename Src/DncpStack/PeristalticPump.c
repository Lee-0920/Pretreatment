 ...
 * @brief 查询指定传感器的工作状态。
 * @param dscp
 * @param data 0-注射器传感器状态  1-进样泵传感器状态
 * @param len
 */
void PeristalticPump_GetSensorStatus(DscpDevice* dscp, Byte* data, Uint16 len)
{
    Uint16 ret = DSCP_OK;
    Uint8 index;
    memcpy(&index, data, sizeof(Uint8));
    TRACE_INFO("\n GetSensorStatus %d", index);
    if(index < DETECTSENSORCONF_TOTAL)
    {
    	if(DetectSensorManager_GetSensor(index))
		{
			TRACE_INFO("\n Sensor Status Busy %d", index);
			ret = DSCP_BUSY;
		}
    	else
		{
			TRACE_INFO("\n Sensor Status Idle %d", index);
		}
    }
    else
    {
    	TRACE_ERROR("\n Error sensor index %d", index);
    }
