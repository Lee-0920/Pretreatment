...
void StaticADControl_Init(void)
{
    ...
    StaticADControl_Gain_InitParam();
...
void StaticADControl_Gain_InitParam(void)
{
    Uint8 buffer[GAIN_CONTROL_SIGN_FLASH_LEN] = { 0 };
    Uint8 paramBuffer[GAIN_CONTROL_PARAM_FLASH_LEN] = {0};
    Uint32 flashFactorySign = 0;
    Uint16 Gain = 0;
    Uint8 i;
    //增益参数初始化
	McuFlash_Read(GAIN_CONTROL_SIGN_FLASH_BASE_ADDR, GAIN_CONTROL_SIGN_FLASH_LEN, buffer);  //读取出厂标志位
	memcpy(&flashFactorySign, buffer, GAIN_CONTROL_SIGN_FLASH_LEN);
	if (FLASH_FACTORY_SIGN == flashFactorySign)       //表示已经过出厂设置
	{//已出厂读取设置
		McuFlash_Read(GAIN_CONTROL_PARAM_FLASH_BASE_ADDR, GAIN_CONTROL_PARAM_FLASH_LEN, paramBuffer);  //读取出厂标志位
		memcpy(&Gain, paramBuffer, GAIN_CONTROL_PARAM_FLASH_LEN);
		OpticalControl_SetGain(Gain);
	}
	else
	{//未出厂,使用设备默认值
		Gain = 1;
		OpticalControl_SetGain(Gain);
		memcpy(paramBuffer, &Gain, GAIN_CONTROL_PARAM_FLASH_LEN);
		//保存设备默认值至Flash
		McuFlash_Write(GAIN_CONTROL_PARAM_FLASH_BASE_ADDR, GAIN_CONTROL_PARAM_FLASH_LEN_TOTAL, paramBuffer);
		//写入出厂标志
		flashFactorySign = FLASH_FACTORY_SIGN;
		memcpy(buffer, &flashFactorySign, GAIN_CONTROL_SIGN_FLASH_LEN);
		McuFlash_Write(GAIN_CONTROL_SIGN_FLASH_BASE_ADDR, GAIN_CONTROL_SIGN_FLASH_LEN, buffer);
	}
}
...
Uint16 StaticADControl_GetDefaultValue(Uint8 index, Bool isExtLED)
{
    ...
    if(index < AD_CONTROLLER_NUM)
    {
        ...
        TRACE_INFO("\n static ad index %d default value %d.", index, value);
        System_NonOSDelay(300);
    }
    else
    {
...
Uint16 StaticADControl_Get_Gain_DefaultValue(void)
{
	Uint8 buffer[GAIN_CONTROL_PARAM_FLASH_LEN] = {0};
	Uint16 value = 0xFFFF;

	McuFlash_Read(GAIN_CONTROL_PARAM_FLASH_BASE_ADDR , sizeof(Uint16), buffer);
	memcpy(&value, buffer, sizeof(Uint16));
	TRACE_INFO("\n static ad ctrl get Gain default value %d.", value);
	return value;
}

void StaticADControl_Set_Gain_DefaultValue(Uint16 value)
{
    Uint8 buffer[MCP4651_CONTROL_PARAM_FLASH_LEN] = {0};
    Uint16 useValue = value;

	memcpy(buffer, &useValue, sizeof(Uint16));
	if (OpticalControl_SetGain(useValue))
	{
		McuFlash_Write(GAIN_CONTROL_PARAM_FLASH_BASE_ADDR, sizeof(Uint16), buffer);
		TRACE_INFO("\n static ad ctrl set Gain default value %d.", useValue);
	}
	else
	{
		TRACE_INFO("\n static ad ctrl set Gain default value fault %d.", useValue);
	}
}
