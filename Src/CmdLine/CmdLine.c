...
const CmdLineEntry g_kConsoleCmdTable[] =
{
	...
	{ "sensor",     Cmd_Sensor, 		"\t\t: Read sensor status"},
...
int Cmd_Gain(int argc, char *argv[])
{
	if(IsEqual(argv[1], "get"))
	{
		StaticADControl_Get_Gain_DefaultValue();
	}
	else if (IsEqual(argv[1], "set"))
	{
		if(argv[2])
		{
			StaticADControl_Set_Gain_DefaultValue(atoi(argv[2]));
		}
		else
		{
			Printf("Lack of param value\n");
		}
	}
	else if (0 == argv[1] || IsEqual(argv[1], "help") || IsEqual(argv[1], "?"))
	{
		Printf("====== Gain Control Commands ======\n");
		Printf(" Gain get   			: get default value from flash \n");
		Printf(" Gain set  [value]		: set default value to flash \n");
	}
	else
	{
		Printf("Invalid param: %s\n", argv[1]);
...
int Cmd_Sensor(int argc, char *argv[])
{
	if (IsEqual(argv[1], "get"))
	{
		if(argv[2])
		{
			if(DetectSensorManager_GetSensor(atoi(argv[2])))
			{
				TRACE_INFO("\n Sensor Status Busy %d", atoi(argv[2]));
			}
			else
			{
				TRACE_INFO("\n Sensor Status Idle %d", atoi(argv[2]));
			}
		}
		else
		{
	...
	{
		Uint8 map = 0;
		for(Uint8 i = 0;i < DETECTSENSORCONF_TOTAL;i++)
		{
			if(DetectSensorManager_GetSensor(i))
			{
				map |= 1 << i;
			}
		}
	...
	{
		Printf("====== Sensor Control Commands ======\n");
		Printf(" sensor get [value]     : read sensor status \n");
		Printf(" sensor map		        : read sensor map status \n");
	}
	else
	{
		Printf("Invalid param: %s\n", argv[1]);
