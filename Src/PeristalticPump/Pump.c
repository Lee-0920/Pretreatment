...
PumpParam Pump_GetMotionParam(Pump* pump)
{
    ...
    if(pump->number >= EXTPUMP_OFFSET)  //扩展的泵
    {
        ...
        //读取FLASH里的校准系数
        McuFlash_Read(PUMP_MOTIONPARAM_FLASH_BASE_ADDR,
            PUMP_MOTIONPARAM_FLASH_LEN, readData);
    }
    else
    {
...
float Pump_GetFactor(Pump* pump)
{
    ...
    if(pump->number >= EXTPUMP_OFFSET)  //扩展的泵
    {
        ...
        //         PUMP_FACTOR_FLASH_LEN, readData);
                //获取FLASH的系数
        McuFlash_Read(PUMP_FACTOR_FLASH_BASE_ADDR,
            PUMP_FACTOR_FLASH_LEN, readData);
    }
    else
    {
...
Bool Pump_Start(Pump* pump, Direction dir, float volume, ParamFLashOp flashOp)
{
    ...
    if(pump->number == 0 || pump->number == 2) //蠕动泵和DC水样泵方向相反
    {
    	if(dir == 0)
    	{
    		PumpDriver_SetDirection(&pump->driver, 1);
    	}
    	else
    	{
    		PumpDriver_SetDirection(&pump->driver, 0);
    	}
    }
    else
    {
